...
const GererTemoignage = () => {

  // États locaux pour gérer le formulaire
  const { temoignages, addTemoignage, editTemoignage } = useContext(TemoignageContext);
  ...
  const handleSubmit = (event) => {
    ...
    if (editing && editedId !== null) {
      editTemoignage({ id: editedId, name, message });
      setEditing(false);
      setEditedId(null);
    } else {
      addTemoignage({ id: Date.now(), name, message });
    }
...
// Rendu JSX du composant de gestion des témoignages
  return (
...
export default GererTemoignage;
